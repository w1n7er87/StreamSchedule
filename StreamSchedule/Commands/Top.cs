...
internal class Top : Command
{
    ...
    private static StringBuilder GetTopByRatio(int skipToPage, bool descending)
    {
        ...
        for (int i = 0; i < topTen.Count; i++)
            result.Append($"{skipToPage + i + 1} {topTen[i].Item1.Username!.Insert(1, BotCore.Invisible)} {MathF.Round(topTen[i].Item2, 3)} er ");
    ...
    private static StringBuilder GetTopByOnline(int skipToPage, bool descending)
    {
        ...
        for (int i = 0; i < topTen.Count; i++)
            result.Append($"{skipToPage + i + 1} {topTen[i].Username!.Insert(1,BotCore.Invisible)} {topTen[i].MessagesOnline} er ");
    ...
    private static StringBuilder GetTopByOffline(int skipToPage, bool descending)
    {
        ...
        for (int i = 0; i < topTen.Count; i++)
            result.Append($"{skipToPage + i + 1} {topTen[i].Username!.Insert(1, BotCore.Invisible)} {topTen[i].MessagesOffline} er ");
    ...
    private static StringBuilder GetTopByTotal(int skipToPage, bool descending)
    {
        ...
        for (int i = 0; i < topTen.Count; i++)
            result.Append($"{skipToPage + i + 1} {topTen[i].Username!.Insert(1, BotCore.Invisible)} {topTen[i].MessagesOffline + topTen[i].MessagesOnline} er ");
