...
internal class Test : Command
{
    ...
    {
        List<string> emoteIDs = BotCore.MessageCache.FirstOrDefault(x => x.Id == message.ID)?.EmoteSet.Emotes.Select(x => x.Id).ToList() ?? [];
        ...
        _ = await BotCore.PagesDB.PageContent.AddAsync(new()
        {
