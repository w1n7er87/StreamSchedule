...
public static class GraphQLClient
{
    ...
    private static readonly HttpClient testHttpClient = new(new SocketsHttpHandler()
        { PooledConnectionLifetime = TimeSpan.FromMinutes(5) });

    private static readonly GraphQLHttpClient client = new(Endpoint, new NewtonsoftJsonSerializer(), httpClient);

    private static readonly GraphQLHttpClient testGqlClient =
        new(Endpoint, new NewtonsoftJsonSerializer(), testHttpClient);
    ...
    public static async Task<bool> VerifyIntegrity(Integrity integrity)
    {
        ...
        GraphQLResponse<QueryResponse?> r =
            await testGqlClient.SendQueryAsync<QueryResponse?>(Queries.IsUsernameAvailable("twitch"));
    ...
    {
        GraphQLResponse<QueryResponse?> result =
            await client.SendQueryAsync<QueryResponse?>(Queries.RequestEmote(emoteID));
        HandleErrors(result.Errors);
    ...
    {
        GraphQLResponse<QueryResponse?> result =
            await client.SendQueryAsync<QueryResponse?>(Queries.RequestEmotesInMessage(messageID));
        HandleErrors(result.Errors);
        if (result.Data?.Message?.Content?.Fragments is null) return [];
        return
            [.. result.Data.Message.Content.Fragments.Where(x => x?.Content is not null).Select(e => e!.Content!.ID)];
    }

    public static async Task<GetUserResult> GetUserByID(string userID)
    {
        GraphQLResponse<QueryResponse?> result =
            await client.SendQueryAsync<QueryResponse?>(Queries.RequestUserByID(userID));
        HandleErrors(result.Errors);
    ...
    {
        GraphQLResponse<QueryResponse?> result =
            await client.SendQueryAsync<QueryResponse?>(Queries.RequestUserByLogin(userLogin));
        HandleErrors(result.Errors);
    ...
    {
        GraphQLResponse<QueryResponse?> result =
            await client.SendQueryAsync<QueryResponse?>(Queries.RequestChatSettings(userID));
        HandleErrors(result.Errors);
